...
if [ "$USE_TMUX" = true ]; then
    ...
    if [ ${#NODES_TO_LAUNCH_SPECS[@]} -gt 0 ]; then
        ...
        for i in "${!NODES_TO_LAUNCH_SPECS[@]}"; do
            if [ "$i" -eq 0 ]; then # Skip the first node as it's already launched
                continue
            fi
            read -r current_package_name current_executable_name <<< "${NODES_TO_LAUNCH_SPECS[$i]}"
